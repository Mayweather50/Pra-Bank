...
class CertificateServiceImplTest {
    ...
    void findByIdPositiveTest() {
        ...
        CertificateDto actualAtmDto = service.findById(id);
    ...
    @Test
    @DisplayName("Создание сертификата, позитивный сценарий")
    void createPositiveTest() {
        ...
        CertificateDto actualDto = service.create(dto);
    ...
    @Test
    @DisplayName("Создание сертификата, ошибка при маппинге, негативный сценарий")
    ...
    void updatePositiveTest() {
        CertificateEntity entity = new CertificateEntity();
        CertificateEntity updatedEntity = new CertificateEntity();
        CertificateDto dto = new CertificateDto();
        CertificateDto updatedDto = new CertificateDto();
    ...
    @Test
    @DisplayName("Обновление сертификата по несуществующему, негативный сценарий")
    void updateNonExistIdNegativeTest() {
        when(repository.findById(1L)).thenReturn(Optional.empty());
        when(supplierNotFound.getException(any(), any()))
                .thenReturn(new EntityNotFoundException("entity not found"));

        assertThrows(EntityNotFoundException.class, () -> service.update(1L, new CertificateDto()));
    }
    ...
    void findAllByIdPositiveTest() {
        ...
        List<CertificateDto> dtoList = List.of(dto1, dto);
        List<CertificateEntity> entityList = List.of(entity1, entity);
        List<Long> ids = List.of(1L, 2L);

        when(repository.findAllById(ids)).thenReturn(entityList);
        when(mapper.toDtoList(entityList)).thenReturn(dtoList);

        List<CertificateDto> actualDtoList = service.findAllById(ids);
